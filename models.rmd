...
\begin{align*}
&P_{i,t}      && \text{Amount of practice in time slot $t$ for item $i$ (number of units of time study)}\\ & && \textbf{NB: This is an integer.}\\
...
Minimize $\underbrace{\sum_{i, t} TIMEPER_i P_{i,t}}_{\text{(1) Overall time spent}} 
        + \underbrace{\sum_{t=1}{(ERGINC_t - ERGDEC_t)}}_{\text{(2) Energy smoothing}}
        + \underbrace{\sum_{i,t}TIMESOFAR_{i,t}}_{\text{(3) Calendar time}}$
...
&& ERG_{t} &= \sum_i{ERGUSED_i \times P_{i,t}}\ \forall t
  && \text{C6: Defn. Used to define increase/decrease for smoothign.}\\
&& ERGINC_{t+1} - ERGDEC_{t+1} &= ERG_{t+1} - ERG_{t}\ \forall t
  && \text{C7: Defn. Change in either direction. One always zero.}\\
\end{align*}

where $P_{i,t} \in \mathbb Z_{\ge 0};\ ERG_{t}, ERGINC_{t}, ERGDEC_{t} \geq 0\ \text{C8: Variable domains}$

### Discussion

The objective function is composed of three positive terms
to be minimized. The first term (1)
captures the amount the sum of the total amount of practice time across all
items. The second term (2) is the relative energy expenditure on practice
...
the diversity with respect to intensity on any given day. 
...
* It's not clear that $TIMESOFAR_{i,t}$ is needed in the model. It's not clear that preference should be given to completing tasks as early as possible. That is, it's
